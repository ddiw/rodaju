...
{{
  "cmd": "명령어",
  "mode": "sorting | stop | standby | resume | (빈 문자열)",
...
<예시>
입력: "분류 시작해"
출력: {{"cmd":"START","priority_order":[],"exclude":[],"raw_text":"분류 시작해"}}

입력: "페트병 먼저 분류해줘"
출력: {{"cmd":"SET_POLICY","priority_order":["plastic"],"exclude":[],"raw_text":"페트병 먼저 분류해줘"}}
...
입력: "플라스틱 캔 순서대로 분류해줘"
출력: {{"cmd":"SET_POLICY","priority_order":["plastic","can","paper"],"exclude":[],"raw_text":"플라스틱 캔 순서대로 분류해줘"}}

입력: "종이 플라스틱 순서대로 분류해줘"
출력: {{"cmd":"SET_POLICY","priority_order":["paper","plastic","can"],"exclude":[],"raw_text":"플라스틱 캔 순서대로 분류해줘"}}

입력: "캔이랑 종이컵만 분류해"
출력: {{"cmd":"SET_POLICY","priority_order":["can","paper"],"exclude":["plastic"],"raw_text":"캔이랑 종이컵만 분류해"}}

입력: "플라스틱만 분류해"
출력: {{"cmd":"SET_POLICY","priority_order":["plastic"],"exclude":["can","paper"],"raw_text":"플라스틱만 분류해"}}

입력: "캔만 분류해"
출력: {{"cmd":"SET_POLICY","priority_order":["can"],"exclude":["plastic","paper"],"raw_text":"캔이랑 종이컵만 분류해"}}

입력: "종이만 분류해"
출력: {{"cmd":"SET_POLICY","priority_order":["paper"],"exclude":["plastic","can"],"raw_text":"종이만 분류해"}}

입력: "잠깐 멈춰"
출력: {{"cmd":"PAUSE","priority_order":[],"exclude":[],"raw_text":"잠깐 멈춰"}}

입력: "다시 시작해"
출력: {{"cmd":"RESUME","priority_order":[],"exclude":[],"raw_text":"다시 시작해"}}

입력: "다 주워", "모두 정리해줘", "정리해"
출력: {{"cmd":"SET_POLICY","priority_or해der":[],"exclude":[],"raw_text":"정리해줘"}}
...
class LLMCommandParser:
    ...
    def parse(self, text: str) -> dict | None:
        ...
        try:
            ...
            index_list = ["plastic","can","paper"]
            exclude_mask = [i.lower() for i in data.get("exclude",[]) if i.lower() in  index_list]
            priority_order = [p.lower() for p in data.get("priority_order", []) if p.lower() in index_list]
...
class VoiceCommandNode(Node):
    ...
    def _publish(self, parsed: dict):
        try:
            ...
            self.get_logger().info(
                f"[CMD 발행] cmd={msg.cmd} "
                f"priority_order={msg.priority_order} exclude={msg.exclude_mask} "
                f"raw='{msg.raw_text}'"
            )
        except Exception as e:
