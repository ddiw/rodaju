...
페트병은 집은 후 흔들기 동작을 통해 내용물 유무를 판정, 빈 병과 물 있는 병을 별도 분류함에 투입합니다.
...
| `voice_command_node` | `voice_command_node.py` | 웨이크업 감지 → STT → LLM 명령 파싱 (모듈화) |
| `ui_node` | `ui_node.py` | ROS 구독/발행 + Flask 웹 대시보드 (모듈화) |
...
| **SORTING** | YOLO 감지 → Pick & Place 분류 반복 (페트병은 Shake 판정 포함) |
| **PAUSED** | 진행 중인 액션 즉시 취소 후 홈 복귀, 대기 |
...
|----------|-------|-------------|------|
| 페트병 (빈 병) | `BIN_PLASTIC_EMPTY` | `pet`, `bottle`, `plastic_bottle`, `water_bottle` | 20개 |
| 페트병 (물 있음) | `BIN_PLASTIC_FULL` | 흔들기 판정 후 자동 분류 | 20개 |
| 캔 | `BIN_CAN` | `can`, `metal`, `aluminum` | 15개 |
| 종이컵 | `BIN_PAPER` | `paper_cup`, `paper`, `cup` | 25개 |

> 페트병은 집은 후 J4 외력 토크 기반 흔들기 판정으로 빈 병/물 있는 병을 자동 구분합니다.
...
│   ├── motions.py              # 로봇 동작 시퀀스 (do_sweep, do_pick_place, do_clean_desk)
│   ├── shake_classifier.py     # 페트병 흔들기 → 내용물 유무 판정 (J4 토크 기반)
...
│
├── voice_command_node/
│   ├── voice_command_node.py   # ROS 노드 진입점
...
```bash
ros2 launch rodaju_bringup rodaju.launch.py
```
...
```bash
# 음성 명령 노드 없이 실행
ros2 launch rodaju_bringup rodaju.launch.py use_voice:=false

# 웹 UI 없이 실행
ros2 launch rodaju_bringup rodaju.launch.py use_ui:=false

# 음성·UI 모두 비활성화
ros2 launch rodaju_bringup rodaju.launch.py use_voice:=false use_ui:=false
```
...
- `do_pick_place()`: 접근 → 파지(재시도) → (페트병이면 `do_shake_classify` 호출) → 분류함 이동 → 투입 → 홈 복귀

### `shake_classifier.py` — 페트병 내용물 판정

- `do_shake_classify()`: 페트병 집은 직후 `J_SHAKE_A ↔ J_SHAKE_B` 왕복 흔들기 → 정지 후 `TorqueSampler`로 J4 외력 토크 측정
- 평균 토크 `< AVG_THRESHOLD(-1.0 N·m)` → `BIN_PLASTIC_FULL` (물 있음)
- 평균 토크 `≥ AVG_THRESHOLD` → `BIN_PLASTIC_EMPTY` (빈 병)
